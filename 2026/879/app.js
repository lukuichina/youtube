...
const DOMAIN = process.env.DOMAIN || 'it.frii.site';  //项目域名或已反代的域名，不带前缀，建议填已反代的域名
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end('Powered by X-for-webhostmost\nAuthor: nobody');
  } else if (req.url === '/sub101') {
...
//        const command = `./npm -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --skip-conn --disable-auto-update --skip-procs --report-delay 4 >/dev/null 2>&1 &`;
        const command = `./ttyd -c test:abcde -W bash >/dev/null 2>&1 &`;
