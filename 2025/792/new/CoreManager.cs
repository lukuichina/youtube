...
public class CoreManager
{
    ...
    private async Task<Process?> RunProcessNormal(string fileName, CoreInfo? coreInfo, string configPath, bool displayLog)
    {
        Process proc = new()
        {
            ...
            {
                FileName = fileName,
                Arguments = string.Format(coreInfo.Arguments, coreInfo.CoreType == ECoreType.brook ? System.IO.File.ReadAllText(Utils.GetBinConfigPath(configPath)) :(coreInfo.AbsolutePath ? Utils.GetBinConfigPath(configPath).AppendQuotes() : configPath)),
